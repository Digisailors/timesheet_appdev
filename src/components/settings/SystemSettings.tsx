  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-8 w-full">
      ...
      <div className="space-y-10">
        ...
        <div>
          <h4 className="text-lg font-semibold text-gray-800 dark:text-gray-100 mb-6">Notification Settings</h4>

          {/* Auto Submit Reminders */}
          <div className="flex items-start justify-between mb-6">
            <div>
              <div className="font-semibold text-base text-gray-900 dark:text-white">Auto Submit Reminders</div>
              <p className="text-gray-500 dark:text-gray-400 text-sm">Send reminders for missing timesheet entries</p>
            </div>
            <div className="relative flex items-center">
              <input type="checkbox" checked={autoSubmit} onChange={() => setAutoSubmit(!autoSubmit)} className="sr-only" />
              <div
                onClick={() => setAutoSubmit(!autoSubmit)}
                className={`w-11 h-6 rounded-full cursor-pointer transition-colors flex items-center ${
                  autoSubmit ? 'bg-blue-600' : 'bg-gray-300 dark:bg-gray-600'
                }`}
              >
                <div
          ...
          <div className="flex items-start justify-between mb-6">
            <div>
              <div className="font-semibold text-base text-gray-900 dark:text-white">Daily Report Emails</div>
              <p className="text-gray-500 dark:text-gray-400 text-sm">Send daily summary reports to administrators</p>
            </div>
            <div className="relative flex items-center">
              <input type="checkbox" checked={dailyReport} onChange={() => setDailyReport(!dailyReport)} className="sr-only" />
              <div
                onClick={() => setDailyReport(!dailyReport)}
                className={`w-11 h-6 rounded-full cursor-pointer transition-colors flex items-center ${
                  dailyReport ? 'bg-blue-600' : 'bg-gray-300 dark:bg-gray-600'
                }`}
              >
                <div
                  className={`w-4 h-4 bg-white rounded-full shadow-md transform transition-transform ${
          ...
          <div className="flex items-start justify-between">
            <div>
              <div className="font-semibold text-base text-gray-900 dark:text-white">Overtime Alerts</div>
              <p className="text-gray-500 dark:text-gray-400 text-sm">Alert when employees log overtime hours</p>
            </div>
            <div className="relative flex items-center">
              <input type="checkbox" checked={overtimeAlert} onChange={() => setOvertimeAlert(!overtimeAlert)} className="sr-only" />
              <div
                onClick={() => setOvertimeAlert(!overtimeAlert)}
                className={`w-11 h-6 rounded-full cursor-pointer transition-colors flex items-center ${
                  overtimeAlert ? 'bg-blue-600' : 'bg-gray-300 dark:bg-gray-600'
                }`}
              >
                <div
                  className={`w-4 h-4 bg-white rounded-full shadow-md transform transition-transform ${
