...
const DesignationSettings: React.FC = () => {
  ...
  const [currentDesignationIndex, setCurrentDesignationIndex] = useState<
    number | null
  >(null);
  ...
  const fetchDesignations = async () => {
    try {
      ...
      if (Array.isArray(data)) {
        ...
        const possibleArrays = Object.values(data).find((value) =>
          Array.isArray(value)
        );
        if (possibleArrays) {
      ...
      console.log("Processed designations:", designationsArray); // Debug log
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isEditing && currentDesignationIndex !== null) {
        ...
        if (!response.ok) {
          throw new Error(
            `Failed to update designation: ${response.statusText}`
          );
        }
        ...
        const response = await fetch(
          `${API_BASE_URL}/designationTypes/create`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        if (!response.ok) {
          throw new Error(
            `Failed to create designation: ${response.statusText}`
          );
        }
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/designationTypes/delete/${designationToDelete.id}`,
        {
          method: "DELETE",
        }
      );
