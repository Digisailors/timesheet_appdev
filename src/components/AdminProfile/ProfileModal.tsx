...
import { getSession } from 'next-auth/react'; // Import from next-auth/react instead
      ...
      try {
        ...
        if (session?.user) {
          ...
          const mappedUserData: UserData = {
            ...
            phone: session.user.phoneNumber || 'Not provided',
            department: '', // You can modify this based on your needs
            joinDate: session.user.createdAt ? new Date(session.user.createdAt).toLocaleDateString('en-GB', {
              day: '2-digit',
              month: 'short',
              year: 'numeric'
            }) : 'N/A',
            lastLogin: session.user.updatedAt ? new Date(session.user.updatedAt).toLocaleDateString('en-GB', {
              day: '2-digit',
              month: 'short',
              year: 'numeric',
              hour: '2-digit',
